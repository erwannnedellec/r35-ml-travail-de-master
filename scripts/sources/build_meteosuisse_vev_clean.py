"""
Prépare les données MétéoSuisse VEV horaires.
...
La station VEV (Vevey/Corseaux, ~380 m) sert de proxy régional pour les
conditions bas de ligne (tronçons dont les deux gares ont ordre <= 10).

Opérations réalisées
--------------------
- lecture des exports historiques VEV horaires ;
...
Sources
-------
- data/external/meteosuisse/ogd-smn_vev_h_historical_2010-2019.csv
- data/external/meteosuisse/ogd-smn_vev_h_historical_2020-2029.csv
- data/external/meteosuisse/ogd-smn_meta_parameters.csv

Sorties
-------
- data/processed/sources/meteo_vev_clean.csv
- data/processed/sources/meteo_vev_clean.parquet
- data/processed/sources/meteo_vev_parameter_quality.csv
"""
...
RAW_METEO_PATHS = [
    Path("data/external/meteosuisse/ogd-smn_vev_h_historical_2010-2019.csv"),
    Path("data/external/meteosuisse/ogd-smn_vev_h_historical_2020-2029.csv"),
]
META_PARAMETERS_PATH = Path("data/external/meteosuisse/ogd-smn_meta_parameters.csv")

OUTPUT_CSV_PATH     = Path("data/processed/sources/meteo_vev_clean.csv")
OUTPUT_PARQUET_PATH = Path("data/processed/sources/meteo_vev_clean.parquet")
OUTPUT_QUALITY_PATH = Path("data/processed/sources/meteo_vev_parameter_quality.csv")
...
def read_raw_meteo() -> pd.DataFrame:
    """Lit et concatène les exports MétéoSuisse VEV bruts."""
    missing_paths = [path for path in RAW_METEO_PATHS if not path.exists()]
    if missing_paths:
        raise FileNotFoundError(f"Fichier(s) météo VEV introuvable(s) : {missing_paths}")
...
def build_clean_meteo(raw_meteo: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Nettoie les mesures VEV et retourne la table propre + rapport qualité."""
    missing_columns = sorted(set(METEO_KEY_COLUMNS) - set(raw_meteo.columns))
    if missing_columns:
        raise ValueError(f"Colonnes manquantes dans les fichiers VEV : {missing_columns}")
    ...
    if unexpected_stations:
        raise ValueError(f"Station(s) inattendue(s) dans les fichiers VEV : {unexpected_stations}")
    ...
    if invalid_timestamp_mask.any():
        ...
        raise ValueError(f"Horodatages VEV invalides : {examples}")
    ...
    for column in value_columns:
        ...
        if invalid_numeric_mask.any():
            ...
            raise ValueError(f"Valeurs invalides dans VEV/{column} : {examples}")
    ...
    if duplicate_clean_names.any():
        ...
        raise ValueError(f"Noms de colonnes propres dupliqués dans VEV : {duplicated}")
    ...
    if duplicate_timestamp_mask.any():
        ...
        raise ValueError(f"Horodatages VEV dupliqués : {examples}")
...
def print_summary(clean: pd.DataFrame, quality: pd.DataFrame) -> None:
    ...
    print("\nNettoyage MétéoSuisse VEV (Vevey/Corseaux) — horaire :")
    ...
    if snow_col in clean.columns:
        ...
        print(f"  observations neige VEV      : {snow_available:,} / {len(clean):,}")
